...
/** True when the AI proxy is reachable. Always true in production builds
 * (Vercel hosts /api/classify alongside the SPA). In dev we assume true
 * when running on a Vercel-style host; otherwise the proxy will simply
 * return a fallback and we'll mark the source as "local". */
export function grokConfigured(): boolean {
  if (typeof window === "undefined") return false;
  if (import.meta.env.PROD) return true;
  // In dev, allow opt-in via a pseudo flag on window for testing.
  return true;
}
...
export async function classifyWithGrok(text: string, signal?: AbortSignal): Promise<ClassifyResult> {
  ...
  try {
    ...
    // If confidence is very low, treat as local fallback for UI honesty.
